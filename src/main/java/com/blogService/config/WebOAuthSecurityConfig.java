...
import com.blogService.repository.OAuth2AuthorizationRequestBasedOnCookieRepository;
...
public class WebOAuthSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http // 토큰 방식으로 인증을 하기 때문에 기존에 사용하던 폼 로그인, 세션 비활성화
                ...
                .authorizeHttpRequests(auth -> auth // 토큰 재발급 URL은 인증 없이 접근 가능하도록 설정, 나머지는 인증 필요
                ...
                .exceptionHandling(exceptionHandling -> exceptionHandling
                        .defaultAuthenticationEntryPointFor(
                                ...
                                (request) -> request.getServletPath().startsWith("/api") // /api로 시작하는 url인 경우 401 상태 코드를 반환하도록 예외 처리
                        ))
                .build();
    }
